...
if ( !function_exists( 'jacqueline_template_testimonials_2_theme_setup' ) ) {
	add_action( 'jacqueline_action_before_init_theme', 'jacqueline_template_testimonials_2_theme_setup', 1 );
	function jacqueline_template_testimonials_2_theme_setup() {
		jacqueline_add_template(array(
			'layout' => 'testimonials-2',
			'template' => 'testimonials-2',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 2/', 'jacqueline')
		));
	}
}

// Template output
if ( !function_exists( 'jacqueline_template_testimonials_2_output' ) ) {
	function jacqueline_template_testimonials_2_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				...
				<div class="sc_testimonial_avatar"><?php jacqueline_show_layout($post_options['photo']); ?></div>
				<?php } ?>
				<div class="sc_testimonial_content"><?php jacqueline_show_layout($post_data['post_content']); ?></div>
