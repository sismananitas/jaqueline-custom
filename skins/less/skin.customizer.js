...
function jacqueline_skin_customizer(option, val) {
	"use strict";
	...
	if (option == 'body_style') {
		...
		var params = JACQUELINE_STORAGE['co_add_params']!=undefined ? JACQUELINE_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#jacqueline-customizer-styles-'+option).length > 0 ? jQuery('#jacqueline-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="jacqueline-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
