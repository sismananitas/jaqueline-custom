...
if ( !function_exists( 'jacqueline_template_team_3_theme_setup' ) ) {
	add_action( 'jacqueline_action_before_init_theme', 'jacqueline_template_team_3_theme_setup', 1 );
	function jacqueline_template_team_3_theme_setup() {
		jacqueline_add_template(array(
			'layout' => 'team-3',
			'template' => 'team-3',
			'mode'   => 'team',
			'title'  => esc_html__('Team /Style 3/', 'jacqueline'),
...
// Template output
if ( !function_exists( 'jacqueline_template_team_3_output' ) ) {
	function jacqueline_template_team_3_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_team_item_avatar"><?php jacqueline_show_layout($post_options['photo']); ?>
					<div class="sc_team_item_hover">
						<a href="<?php echo esc_url($post_options['link']); ?>" class="sc_team_item_icon icon-email19"></a>
					</div>
				</div>
				<div class="sc_team_item_info">
					...
					<div class="sc_team_item_position"><?php jacqueline_show_layout($post_options['position']);?></div>
				</div>
			</div>
		<?php
