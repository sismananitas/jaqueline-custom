...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		var tab = jQuery(this).parents('.jacqueline_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.jacqueline_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && jacqueline_get_cookie('to_demo_fixed')==1) jacqueline_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.jacqueline_options_field_info .to_demo_pin', function(e) {
		jacqueline_options_fix_demo(jQuery(this).parents('.jacqueline_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	jacqueline_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.jacqueline_options_input,.jacqueline_options_field input[type="hidden"]', function(e) {
		var tab = jQuery(this).parents('.jacqueline_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		jacqueline_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.jacqueline_options_input_color', function(e) {
		var tab = jQuery(this).parents('.jacqueline_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		jacqueline_options_decorate_demo(scheme);
	...
	jQuery('#partition_schemes_content .jacqueline_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.jacqueline_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? JACQUELINE_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.jacqueline_options_tab').find('> .jacqueline_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="jacqueline_options_schemes_order" class="jacqueline_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function jacqueline_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.jacqueline_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.jacqueline_options').find('>.jacqueline_options_field_info');
...
function jacqueline_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.jacqueline_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	jacqueline_message_confirm(JACQUELINE_STORAGE['to_strings']['scheme_delete_confirm'], JACQUELINE_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'jacqueline_options_scheme_delete',
			nonce: JACQUELINE_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.jacqueline_options_tab').find('> .jacqueline_options_schemes_order').val()
		};
		setTimeout(function(){
			jacqueline_message_info(JACQUELINE_STORAGE['to_strings']['recompile_styles'], JACQUELINE_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(JACQUELINE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				jacqueline_message_success(JACQUELINE_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+JACQUELINE_STORAGE['to_strings']['reload_page'], JACQUELINE_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				jacqueline_message_warning(JACQUELINE_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, JACQUELINE_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function jacqueline_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.jacqueline_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	jacqueline_message_confirm(JACQUELINE_STORAGE['to_strings']['scheme_copy_confirm'], JACQUELINE_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'jacqueline_options_scheme_copy',
			nonce: JACQUELINE_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.jacqueline_options_tab').find('> .jacqueline_options_schemes_order').val()
		};
		...
		jQuery.post(JACQUELINE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				jacqueline_message_success(JACQUELINE_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+JACQUELINE_STORAGE['to_strings']['reload_page'], JACQUELINE_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				jacqueline_message_warning(JACQUELINE_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, JACQUELINE_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function jacqueline_options_fix_demo(info) {
	"use strict";
	var info2 = info.parents('.jacqueline_options').toggleClass('to_demo_fixed').find('>.jacqueline_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.jacqueline_options').append(clonedObj);
		info2 = info.parents('.jacqueline_options').find('>.jacqueline_options_field_info');
	}
	jacqueline_set_cookie('to_demo_fixed', jQuery('.jacqueline_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function jacqueline_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.jacqueline_options_field_info').data('scheme');
